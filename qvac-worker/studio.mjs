...
import { validateStructuredVision, conservativeVisionFallback, reconcileV3Events, selectDeepV3StoryEvents, cleanNaturalLanguageObservation, isVisionSchemaEcho } from './public/deep-video-v3.js'
...
export async function analyseDeepWindow(input={},visionCall=callVisionPsy){
  ...
  const language=input.language==='it'?'it':'en',semanticHint=String(input.interval?.semantic_hint||'').slice(0,300),grounding=language==='it'
    ? `Osserva la sequenza cronologica nell'intervallo ${interval.start.toFixed(2)}–${interval.end.toFixed(2)} secondi. Soggetti consentiti: ${knownSubjects.join(', ')||'nessuno'}. Restituisci solo un piccolo JSON con gli eventi visibili. Per ogni evento servono attore, azione, confidenza e una breve osservazione; target è facoltativo. Non inventare emozioni o movimento della fotocamera. Se nulla è affidabile restituisci {"events":[]}. ${semanticHint} ${deepV3ActionInstruction} Esempio di formato: ${deepV3Schema}`
    : `Observe the chronological sequence in interval ${interval.start.toFixed(2)}–${interval.end.toFixed(2)} seconds. Allowed subjects: ${knownSubjects.join(', ')||'none'}. Return only a small JSON object containing visible events. Each event needs actor, action, confidence and a short observation; target is optional. Do not infer emotions or camera movement. Return {"events":[]} when nothing is reliable. ${semanticHint} ${deepV3ActionInstruction} Format example: ${deepV3Schema}`
