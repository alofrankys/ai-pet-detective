...
export function buildDeepVisionContent(sequence={},grounding=''){const frames=(Array.isArray(sequence.frames)?sequence.frames:[]).filter(frame=>Number.isFinite(Number(frame?.timestamp))&&/^data:image\/jpeg;base64,/.test(String(frame?.image||''))).sort((a,b)=>Number(a.timestamp)-Number(b.timestamp));if(frames.length>=2){const content=[];for(const [index,frame] of frames.entries()){content.push({type:'text',text:`Frame ${index+1}/${frames.length} · ${Number(frame.timestamp).toFixed(2)}s · ordered earliest to latest`},{type:'image_url',image_url:{url:frame.image}})}content.push({type:'text',text:grounding});return {content,visual_input:{mode:'multi_image',frame_count:frames.length,timestamps:frames.map(frame=>Number(frame.timestamp)),frame_dimensions:frames.map(frame=>({width:Number(frame.width)||null,height:Number(frame.height)||null}))}}}if(/^data:image\/jpeg;base64,/.test(String(sequence.image||'')))return {content:[{type:'image_url',image_url:{url:sequence.image}},{type:'text',text:grounding}],visual_input:{mode:'contact_sheet_fallback',frame_count:Number(sequence.frame_count)||1,timestamps:Array.isArray(sequence.timestamps)?sequence.timestamps:[],frame_dimensions:Array.isArray(sequence.frame_dimensions)?sequence.frame_dimensions:[]}};throw new Error('Deep window requires at least two ordered JPEG frames')}
...
export async function analyseDeepWindow(input={},visionCall=callVisionPsy){
  ...
  const visualMessage=buildDeepVisionContent(input.sequence,grounding);if(visualMessage.visual_input.mode==='contact_sheet_fallback')decisions.push({status:'accepted',stage:'visual_input',reason:'contact_sheet_fallback',interval})
  metrics.vision_calls++;const rawResponse=await visionCall([{role:'user',content:visualMessage.content}],420);if(usefulVisionText(rawResponse))metrics.useful_vision_responses++
