...
const deepV3Schema='{"events":[{"actor_ref":"subject_1","action":"petting","target_ref":"person_1","confidence":0.82,"description":"A person repeatedly strokes subject_1."}]}'

function deepV3Metrics(){return {vision_calls:0,useful_vision_responses:0,valid_structured_responses:0,valid_structured_with_events:0,valid_structured_empty:0,repaired_responses:0,fallback_parsed_responses:0,discarded_responses:0,structured_events_created:0}}
...
export async function analyseDeepWindow(input={},visionCall=callVisionPsy){
  ...
  const language=input.language==='it'?'it':'en',semanticHint=String(input.interval?.semantic_hint||'').slice(0,300),visualScope=language==='it'
    ...
    : 'Inspect for any clearly visible meaningful pet behavior: posture change, jumping on or off, person-dog contact, dog-dog interaction, visible object presentation, mouth contact with an object, pickup, holding or carrying, playing, tugging, or dropping. Detector/object telemetry is a supporting signal only. If a clearly visible persistent object interaction is present in the frames, report it even if no detector object candidate exists. Do not invent an object when not visually clear.',grounding=language==='it'
    ? `Osserva la sequenza cronologica nell'intervallo ${interval.start.toFixed(2)}–${interval.end.toFixed(2)} secondi. Soggetti consentiti: ${knownSubjects.join(', ')||'nessuno'}. Restituisci solo un piccolo JSON con gli eventi visibili. Per ogni evento servono attore, azione, confidenza e una breve osservazione; target è facoltativo. Non inventare emozioni o movimento della fotocamera. Se nulla è affidabile restituisci {"events":[]}. ${visualScope} ${semanticHint} ${deepV3ActionInstruction} Esempio di formato: ${deepV3Schema}`
    : `Observe the chronological sequence in interval ${interval.start.toFixed(2)}–${interval.end.toFixed(2)} seconds. Allowed subjects: ${knownSubjects.join(', ')||'none'}. Return only a small JSON object containing visible events. Each event needs actor, action, confidence and a short observation; target is optional. Do not infer emotions or camera movement. Return {"events":[]} when nothing is reliable. ${visualScope} ${semanticHint} ${deepV3ActionInstruction} Format example: ${deepV3Schema}`
  metrics.vision_calls++;const rawResponse=await visionCall([{role:'user',content:[{type:'image_url',image_url:{url:image}},{type:'text',text:grounding}]}],420);if(usefulVisionText(rawResponse))metrics.useful_vision_responses++
  ...
  if(!validated.valid&&!initialSchemaEcho){
    ...
    const repairPrompt=language==='it'?`Ripara l'output seguente in JSON rigorosamente valido senza aggiungere fatti. Usa soltanto soggetti consentiti e limita i timestamp a ${interval.start}–${interval.end}. Output malformato: ${String(rawResponse).slice(0,5000)}. ${deepV3ActionInstruction} Struttura JSON: ${deepV3Schema}`:`Repair the following output into strict valid JSON without adding facts. Use only allowed subjects and keep timestamps within ${interval.start}–${interval.end}. Malformed output: ${String(rawResponse).slice(0,5000)}. ${deepV3ActionInstruction} JSON structure: ${deepV3Schema}`
  ...
  const visionWindow={id:input.interval?.id||null,start:interval.start,end:interval.end,reasons:Array.isArray(input.interval?.reasons)?input.interval.reasons:[],subjects:Array.isArray(input.interval?.subjects)?input.interval.subjects:[],salience:Number(input.interval?.salience)||0,raw_response:String(rawResponse||'').slice(0,8000),parsed_response:parsedResponse,validation_result:{valid:validated.valid,event_count:validated.events.length,context:validated.context||null},validated_events:validated.events,validation_errors:validated.errors||[],initial_validation_errors:initialValidation.errors||[],repair_attempted:repairAttempted,repaired_raw_response:repairedRawResponse===null?null:String(repairedRawResponse).slice(0,8000),repaired_parsed_response:repairedParsedResponse,repaired_validation_result:repairedValidation?{valid:repairedValidation.valid,event_count:repairedValidation.events.length,errors:repairedValidation.errors}:null,schema_echo_detected:initialSchemaEcho||isVisionSchemaEcho(repairedRawResponse),fallback_used:fallbackUsed}
