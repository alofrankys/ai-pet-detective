...
import {
  ...
  PersistentHandDogRelations,
  PersistentObjectInteractionEvidence,
...
const deepProgressPhases=[{title:'deepPassA',status:'deepScanning',start:0,end:58},{title:'deepPassB',status:'deepFinding',start:58,end:66},{title:'deepPassC',status:'deepReviewing',start:66,end:94},{title:'deepPassD',status:'deepWriting',start:94,end:100}]
function setDeepProgress(phaseIndex,completed,total,detail=''){
  const phase=deepProgressPhases[phaseIndex]||deepProgressPhases[0],ratio=total?Math.max(0,Math.min(1,completed/total)):0,percent=Math.round(phase.start+(phase.end-phase.start)*ratio)
  deepProgress.hidden=false;deepPhase.textContent=t(phase.title);deepPercent.textContent=`${percent}%`;deepProgressBar.value=percent;deepDetail.textContent=detail||t(phase.status)
  deepStepElements.forEach((element,index)=>{element.textContent=t(deepProgressPhases[index].title);element.classList.toggle('complete',index<phaseIndex);element.classList.toggle('active',index===phaseIndex)})
}
...
async function deepSemanticFrameSequence(interval){
  const timestamps=orderedSemanticFrameTimestamps(interval,6),dimensions=fitAspectPreservingDimensions(video.videoWidth,video.videoHeight,640),layout=aspectPreservingContactSheetLayout(timestamps.map(timestamp=>({timestamp,...dimensions}))),frameCanvas=document.createElement('canvas'),frameContext=frameCanvas.getContext('2d'),sheetCanvas=document.createElement('canvas'),sheetContext=sheetCanvas.getContext('2d');frameCanvas.width=dimensions.width;frameCanvas.height=dimensions.height;sheetCanvas.width=layout.width;sheetCanvas.height=layout.height;sheetContext.fillStyle='#07100b';sheetContext.fillRect(0,0,layout.width,layout.height)
...
async function runDeepRecordedAnalysis(){
  ...
  narrativeVersion.textContent='VISIONPSY · DEEP ANALYSIS';deepDebugRecorder=new DeepVideoDebugRecorder({source:currentSource});debugRecorder.meta={...debugRecorder.meta,analysis_mode:'recorded_deep_v3'}
  const sampleFps=duration>300?5:6,step=1/sampleFps,totalFrames=Math.max(1,Math.floor(duration/step)+1),identity=new PersistentIdentityManager({newSubjectFrames:Math.max(8,Math.round(sampleFps*1.8))}),camera=new BackgroundMotionEstimator(),surfaces=new PersistentSurfaceMap(),handRelations=new PersistentHandDogRelations(),objectRelations=new PersistentObjectInteractionEvidence(),posture=new TemporalPostureClassifier(),fragments=new Map(),nextFragment={value:1},previousSubjects=new Map(),objectFirstSeen=new Map(),observations=[],semanticEvents=[];let poseFrames=0,cameraConfidenceSum=0
  setDeepProgress(0,0,totalFrames,t('deepPreparing'))
  for(let frameIndex=0;frameIndex<totalFrames;frameIndex++){
    ...
    let hands=[];if(handLandmarker&&stableDogs.length){try{const result=handLandmarker.detectForVideo(capture,time*1000);hands=(result.landmarks||[]).map(points=>points.map(point=>({x:point.x,y:point.y,z:point.z})))}catch{}}
    const knownObjectCandidates=subjects.filter(item=>dogInterestLabels.has(item.label)).map(item=>{if(!objectFirstSeen.has(item.track_id))objectFirstSeen.set(item.track_id,time);const dog=stableDogs.find(candidate=>candidate.pose?.keypoints?.[2]&&pointBoxDistance(candidate.pose.keypoints[2],item.box)<.08);return {track_id:item.track_id,label:item.label,box:item.box,subject_id:dog?.subject_id||null,confidence:item.score,new:time-objectFirstSeen.get(item.track_id)<.5,mouth_proximity:Boolean(dog)}}),objectCandidates=[...knownObjectCandidates]
    for(const dog of stableDogs){const contactingHands=hands.filter(points=>points.some(point=>point.x>=dog.box[0]-.04&&point.x<=dog.box[2]+.04&&point.y>=dog.box[1]-.04&&point.y<=dog.box[3]+.04)),contact=contactingHands.length>0,mouth=dog.pose?.keypoints?.[2]||{x:(dog.box[0]+dog.box[2])/2,y:dog.box[1]+(dog.box[3]-dog.box[1])*.2},handPoint=contactingHands.flat().sort((a,b)=>Math.hypot(a.x-mouth.x,a.y-mouth.y)-Math.hypot(b.x-mouth.x,b.y-mouth.y))[0]||null,knownObject=knownObjectCandidates.some(item=>item.subject_id===dog.subject_id&&item.mouth_proximity),objectSignal=objectRelations.update({subjectId:dog.subject_id,time,handPoint,mouthPoint:mouth,knownObject}),objectEvidence=knownObject||objectSignal.active,signal=handRelations.update({subjectId:dog.subject_id,time,contact,handPoints:contactingHands,dogBox:dog.box,objectEvidence});if(objectSignal.candidate)objectCandidates.push(objectSignal.candidate);if(contact)relations.push({type:'hand_dog_interaction',first:dog.subject_id,second:'person_1',distance:handPoint?Math.hypot(handPoint.x-mouth.x,handPoint.y-mouth.y):0,changed:true,persistent:signal.persistent,petting:signal.pettingActive,object_interaction:objectEvidence});if(signal.event){frameEvents.push(signal.event);semanticEvents.push(signal.event)}}
  ...
  for(let index=0;index<candidates.length;index++){
    ...
    try{const sequence=await deepSemanticFrameSequence(candidate),knownSubjects=[...identity.subjects.keys(),'person_1'],windowObservations=observations.filter(item=>item.time>=candidate.start&&item.time<=candidate.end),overlappingEvents=semanticEvents.filter(event=>event.end>=candidate.start&&event.start<=candidate.end),evidence={hand_dog_subjects:[...new Set(windowObservations.flatMap(item=>(item.relations||[]).filter(relation=>relation.type==='hand_dog_interaction'&&relation.persistent).map(relation=>relation.first)).filter(Boolean))],surface_subjects:[...new Set(overlappingEvents.filter(event=>event.source==='surface-v3').map(event=>event.actor).filter(Boolean))],object_subjects:[...new Set(windowObservations.flatMap(item=>(item.object_candidates||[]).filter(object=>object.mouth_proximity||object.persistent||object.hand_proximity).map(object=>object.subject_id)).filter(Boolean))],deterministic_events:overlappingEvents.map(event=>({actor:event.actor,action:event.action,start:event.start,end:event.end,source:event.source}))},response=await fetch('/api/deep/analyse-window',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({language,interval:candidate,sequence,knownSubjects,evidence,context:currentContextPayload()})}),result=await response.json();if(!response.ok)throw new Error(result.error||'Deep semantic window failed');deepDebugRecorder.recordVision(result,candidate);semanticEvents.push(...(result.events||[]));setDeepProgress(2,index+1,candidates.length,`${t('deepReviewing')} ${index+1} / ${candidates.length}`)}catch(error){deepDebugRecorder.dropped_events.push({status:'rejected',stage:'window_request',interval:{start:candidate.start,end:candidate.end},reason:String(error.message||error)});setDeepProgress(2,index+1,candidates.length,`${t('deepReviewing')} ${index+1} / ${candidates.length}`)}
  }
...
function renderDeepSessionFacts(events){const duration=Math.max(0,currentSessionDuration()),minutes=Math.floor(duration/60),seconds=Math.round(duration%60),dogs=deepDebugRecorder?.identity_metrics.persistent_subjects||0,facts=[`${t('fullVideoAnalysed')} · ${minutes}:${String(seconds).padStart(2,'0')}`,`${dogs} ${t('dogsFollowed')}`,`${events.length} ${t('keyMoments')}`];sessionSummaryFacts.replaceChildren(...facts.map(text=>Object.assign(document.createElement('span'),{textContent:text})))}
