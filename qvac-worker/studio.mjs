...
import { reconcileV3Events, selectDeepV3StoryEvents, naturalObservationsToEvents, isVisionSchemaEcho, cleanNaturalLanguageObservation } from './public/deep-video-v3.js'
...
function deepV3Metrics(){return {vision_windows_requested:0,vision_windows_completed:0,observations_returned:0,observations_canonicalized:0,observations_ambiguous:0,semantic_events_created:0}}

export function buildDeepVisionContent(sequence={},grounding='',mode='multi_image'){const frames=(Array.isArray(sequence.frames)?sequence.frames:[]).filter(frame=>Number.isFinite(Number(frame?.timestamp))&&/^data:image\/jpeg;base64,/.test(String(frame?.image||''))).sort((a,b)=>Number(a.timestamp)-Number(b.timestamp));if(mode==='multi_image'&&frames.length>=2){const content=[];for(const [index,frame] of frames.entries()){content.push({type:'text',text:`Frame ${index+1}/${frames.length} · ${Number(frame.timestamp).toFixed(2)}s · ordered earliest to latest`},{type:'image_url',image_url:{url:frame.image}})}content.push({type:'text',text:grounding});return {content,visual_input:{mode:'multi_image',frame_count:frames.length,timestamps:frames.map(frame=>Number(frame.timestamp)),frame_dimensions:frames.map(frame=>({width:Number(frame.width)||null,height:Number(frame.height)||null}))}}}const sheet=sequence.contact_sheet||sequence;if(/^data:image\/jpeg;base64,/.test(String(sheet.image||'')))return {content:[{type:'image_url',image_url:{url:sheet.image}},{type:'text',text:grounding}],visual_input:{mode:'contact_sheet',frame_count:Number(sheet.frame_count)||frames.length||1,timestamps:Array.isArray(sheet.timestamps)?sheet.timestamps:frames.map(frame=>Number(frame.timestamp)),frame_dimensions:Array.isArray(sheet.frame_dimensions)?sheet.frame_dimensions:frames.map(frame=>({width:Number(frame.width)||null,height:Number(frame.height)||null}))}};throw new Error(mode==='multi_image'?'Deep window requires at least two ordered JPEG frames':'Deep window contact sheet unavailable')}

export async function analyseDeepWindow(input={},visionCall=callVisionPsy){
  const metrics=deepV3Metrics(),decisions=[],interval={start:Math.max(0,Number(input.interval?.start)||0),end:Math.max(0,Number(input.interval?.end)||0)}
  const grounding='These frames are chronological from earliest to latest. Describe up to 3 clearly visible meaningful actions or changes involving the dogs. Use one short factual sentence per observation. Describe only visible behavior, not emotion or intent. Mention a person, another dog, furniture, or an object only when relevant. Pay attention to changes involving posture, movement, contact, mouth/object interaction and play. If nothing meaningful is clearly visible, output NONE.'
  const visualMessage=buildDeepVisionContent(input.sequence,grounding,'contact_sheet_fallback');metrics.vision_windows_requested=1
  const rawResponse=await visionCall([{role:'user',content:visualMessage.content}],240,{visual_mode:'contact_sheet',image_count:1});metrics.vision_windows_completed=1
  const normalized=naturalObservationsToEvents(rawResponse,{interval,candidateSubjects:Array.isArray(input.interval?.subjects)?input.interval.subjects:[],evidence:input.evidence||{}}),events=normalized.events
  metrics.observations_returned=normalized.observations.length;metrics.observations_canonicalized=normalized.canonicalized.filter(item=>item.actions.length).length;metrics.observations_ambiguous=normalized.actorResolution.ambiguous?normalized.canonicalized.filter(item=>item.actions.length).length:0;metrics.semantic_events_created=events.length
  decisions.push(...events.map(event=>({status:'accepted',stage:'natural_language_canonicalization',event_id:event.id,reason:'recognized_visible_action'})))
  for(const item of normalized.canonicalized.filter(item=>!item.actions.length))decisions.push({status:'rejected',stage:'natural_language_canonicalization',reason:'no_conservative_action_match',observation:item.observation,interval})
  const visionWindow={id:input.interval?.id||null,start:interval.start,end:interval.end,reasons:Array.isArray(input.interval?.reasons)?input.interval.reasons:[],subjects:Array.isArray(input.interval?.subjects)?input.interval.subjects:[],salience:Number(input.interval?.salience)||0,visual_input:visualMessage.visual_input,raw_response:String(rawResponse??''),raw_observations:String(rawResponse??''),observations:normalized.observations,canonicalized_observations:normalized.canonicalized,actor_resolution:normalized.actorResolution,semantic_events:events}
  return {version:3,events,context:{},metrics,decisions,repair_attempted:false,raw_response:visionWindow.raw_response,vision_window:visionWindow}
}
