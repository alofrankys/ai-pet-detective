...
export function orderedSemanticFrameTimestamps(interval={},count=8){const start=Math.max(0,Number(interval.start)||0),end=Math.max(start,Number(interval.end)||start),total=Math.max(2,Math.min(8,Math.round(count)||8));return Array.from({length:total},(_,index)=>Number((start+(end-start)*(index/(total-1))).toFixed(3)))}
export function fitAspectPreservingDimensions(width,height,longEdge=640){const sourceWidth=Math.max(1,Number(width)||1),sourceHeight=Math.max(1,Number(height)||1),scale=Math.min(1,Math.max(1,Number(longEdge)||640)/Math.max(sourceWidth,sourceHeight));return {width:Math.max(1,Math.round(sourceWidth*scale)),height:Math.max(1,Math.round(sourceHeight*scale))}}
export function aspectPreservingContactSheetLayout(frames=[],{columns=2,cellLongEdge=480}={}){const ordered=[...frames].sort((a,b)=>Number(a.timestamp)-Number(b.timestamp)),columnCount=Math.max(1,Math.min(columns,ordered.length||1)),first=ordered[0]||{width:16,height:9},cell=fitAspectPreservingDimensions(first.width,first.height,cellLongEdge),rows=Math.max(1,Math.ceil(ordered.length/columnCount));return {width:cell.width*columnCount,height:cell.height*rows,cell_width:cell.width,cell_height:cell.height,columns:columnCount,rows,placements:ordered.map((frame,index)=>({index,timestamp:Number(frame.timestamp),x:index%columnCount*cell.width,y:Math.floor(index/columnCount)*cell.height,width:cell.width,height:cell.height}))}}
...
export function isVisionSchemaEcho(value){
  ...
  return PIPE_ACTION_LIST.test(text)||/<\/?(?:action|actor|subject|allowed_action|placeholder)>|<visible observation>|\b(?:schema|template|placeholder|example json)\b/i.test(text)||/"action"\s*:\s*"(?:observed_action|allowed action|action_name|<allowed_action>|\.\.\.)"/i.test(text)||/"evidence"\s*:\s*\[\s*"(?:short observable reason|observable evidence|\.\.\.)"\s*\]/i.test(text)||/"confidence"\s*:\s*0\.82[\s\S]*"description"\s*:\s*"A person repeatedly strokes subject_1\."/i.test(text)
}
...
export class PersistentSurfaceMap{
  constructor({confirmFrames=4,switchMargin=.22,relationFrames=3,relationSeconds=.35,initialFloorFrames=6,initialFloorSeconds=1.5,cooldownSeconds=1.25}={}){Object.assign(this,{confirmFrames,switchMargin,relationFrames,relationSeconds,initialFloorFrames,initialFloorSeconds,cooldownSeconds});this.entities=new Map();this.relations=new Map();this.nextId=1}
  ...
  updateRelation(subject,time){
    if(!subject.subject_id)return null;const candidate=this.relationFor(subject.box),state=this.relations.get(subject.subject_id)||{stable:null,pending:null,frames:0,since:time,transitionReady:false,floorReadyFrames:0,floorReadySince:null}
    if(candidate===state.stable){state.pending=null;state.frames=0;if(candidate==='floor'&&!state.transitionReady&&this.entities.size){if(state.floorReadySince===null)state.floorReadySince=time;state.floorReadyFrames++;if(state.floorReadyFrames>=this.initialFloorFrames&&time-state.floorReadySince>=this.initialFloorSeconds)state.transitionReady=true}this.relations.set(subject.subject_id,state);return null}
    ...
    const previous=state.stable,previousReady=state.transitionReady,confirmationFrames=state.frames;state.stable=candidate;state.since=state.pendingSince;state.pending=null;state.frames=0;state.floorReadyFrames=0;state.floorReadySince=null;state.transitionReady=candidate.startsWith('on:')||previousReady;this.relations.set(subject.subject_id,state);if(!previous||!previousReady)return null
...
export class PersistentHandDogRelations{
  constructor({confirmFrames=6,confirmSeconds=.85,maxGapSeconds=.45,minTravel=.055,minDirectionChanges=2,restartGapSeconds=.8,cooldownSeconds=1.5}={}){Object.assign(this,{confirmFrames,confirmSeconds,maxGapSeconds,minTravel,minDirectionChanges,restartGapSeconds,cooldownSeconds});this.states=new Map()}
  update({subjectId,time,contact=false,handPoints=[],dogBox=null,objectEvidence=false}={}){
    if(!subjectId)return {interaction:false,pettingActive:false,event:null}
    const points=handPoints.flat().filter(point=>Number.isFinite(point?.x)&&Number.isFinite(point?.y)),centroid=points.length?{x:points.reduce((sum,point)=>sum+point.x,0)/points.length,y:points.reduce((sum,point)=>sum+point.y,0)/points.length}:null,width=Math.max(.04,Number(dogBox?.[2])-Number(dogBox?.[0])),height=Math.max(.04,Number(dogBox?.[3])-Number(dogBox?.[1])),relative=centroid&&dogBox?{x:(centroid.x-dogBox[0])/width,y:(centroid.y-dogBox[1])/height}:centroid,state=this.states.get(subjectId)||{frames:0,contactFrames:0,start:time,contactStart:time,lastContact:-Infinity,travel:0,lastRelative:null,lastDelta:null,directionChanges:0,emitted:false,lastEventAt:-Infinity,pettingActive:false}
    if(!contact){if(time-state.lastContact>this.restartGapSeconds){state.frames=0;state.contactFrames=0;state.travel=0;state.lastRelative=null;state.lastDelta=null;state.directionChanges=0;state.emitted=false;state.pettingActive=false}this.states.set(subjectId,state);return {interaction:false,persistent:false,pettingActive:false,event:null}}
    if(time-state.lastContact>this.maxGapSeconds){state.frames=0;state.contactFrames=0;state.start=time;state.contactStart=time;state.travel=0;state.lastRelative=null;state.lastDelta=null;state.directionChanges=0;state.pettingActive=false}
    state.contactFrames++;const persistent=state.contactFrames>=3&&time-state.contactStart>=.3
    if(objectEvidence){state.frames=0;state.start=time;state.travel=0;state.lastRelative=relative;state.lastDelta=null;state.directionChanges=0;state.pettingActive=false;state.lastContact=time;this.states.set(subjectId,state);return {interaction:true,persistent,objectInteraction:true,pettingActive:false,event:null}}
    if(!state.frames)state.start=time;state.frames++;if(relative&&state.lastRelative){const delta={x:relative.x-state.lastRelative.x,y:relative.y-state.lastRelative.y},magnitude=Math.hypot(delta.x,delta.y);state.travel+=magnitude;if(magnitude>.012&&state.lastDelta){const prior=Math.hypot(state.lastDelta.x,state.lastDelta.y),dot=prior?((delta.x*state.lastDelta.x+delta.y*state.lastDelta.y)/(magnitude*prior)):1;if(dot<-.2)state.directionChanges++}if(magnitude>.006)state.lastDelta=delta}state.lastRelative=relative;state.lastContact=time
    const qualifies=!objectEvidence&&state.frames>=this.confirmFrames&&time-state.start>=this.confirmSeconds&&state.travel>=this.minTravel&&state.directionChanges>=this.minDirectionChanges;state.pettingActive=qualifies||state.pettingActive&&!objectEvidence
    let event=null;if(qualifies&&!state.emitted&&time-state.lastEventAt>=this.cooldownSeconds){state.emitted=true;state.lastEventAt=time;event=makeEvent({id:`hand_${subjectId}_${Math.round(time*1000)}`,start:state.start,end:time,actor:subjectId,action:'petting',target:'person_1',confidence:.78,importance:.92,description:`A person repeatedly strokes ${subjectId}.`,source:'relation-v3',evidence:{hand_contact_frames:state.frames,relative_hand_travel:state.travel,direction_changes:state.directionChanges}})}this.states.set(subjectId,state);return {interaction:true,persistent,pettingActive:state.pettingActive,event}
  }
  reset(){this.states.clear()}
}

export class PersistentObjectInteractionEvidence{
  constructor({confirmFrames=4,confirmSeconds=.6,maxGapSeconds=.5,maxDistanceSpread=.055}={}){Object.assign(this,{confirmFrames,confirmSeconds,maxGapSeconds,maxDistanceSpread});this.states=new Map()}
  update({subjectId,time,handPoint=null,mouthPoint=null,knownObject=false}={}){if(!subjectId)return {active:false,candidate:null};const separation=handPoint&&mouthPoint?distance(handPoint,mouthPoint):Infinity,near=separation>=.018&&separation<.11,state=this.states.get(subjectId)||{frames:0,start:time,last:-Infinity,distances:[],active:false,label:'unknown_object'};if(!near){const disappeared=state.active,label=state.label;state.frames=0;state.start=time;state.distances=[];state.active=false;state.last=time;this.states.set(subjectId,state);return {active:false,candidate:disappeared?{track_id:`unknown_object:${subjectId}`,label,subject_id:subjectId,confidence:.56,disappeared:true,hand_proximity:true,mouth_proximity:true}:null}}if(time-state.last>this.maxGapSeconds){state.frames=0;state.start=time;state.distances=[];state.active=false}if(!state.frames)state.start=time;state.frames++;state.last=time;state.label=knownObject?'object':'unknown_object';state.distances.push(separation);if(state.distances.length>12)state.distances.shift();const spread=Math.max(...state.distances)-Math.min(...state.distances);state.active=state.frames>=this.confirmFrames&&time-state.start>=this.confirmSeconds&&spread<=this.maxDistanceSpread;this.states.set(subjectId,state);return {active:state.active,candidate:state.active?{track_id:`unknown_object:${subjectId}`,label:state.label,subject_id:subjectId,confidence:knownObject ? .78 : .58,new:state.frames===this.confirmFrames,persistent:true,hand_proximity:true,mouth_proximity:true}:null}}
  reset(){this.states.clear()}
}
...
export function generateCandidateIntervals(observations=[],duration=0,{coverageSeconds=12}={}){
  ...
  for(let index=1;index<observations.length;index++){const previous=observations[index-1],current=observations[index],reasons=[],subjects=[]
    ...
    const pettingSignals=current.relations?.filter(item=>item.type==='hand_dog_interaction'&&item.petting)||[];if(pettingSignals.length){reasons.push('petting_signal');subjects.push(...pettingSignals.map(item=>item.first).filter(Boolean))}
    const genericHands=current.relations?.filter(item=>item.type==='hand_dog_interaction'&&item.persistent)||[];if(genericHands.length){reasons.push('hand_dog_interaction');subjects.push(...genericHands.map(item=>item.first).filter(Boolean))}
    if(current.relations?.some(item=>item.changed))reasons.push('relationship_change')
    if(current.object_candidates?.some(item=>item.new||item.disappeared||item.persistent||item.hand_proximity||item.mouth_proximity))reasons.push('object_interaction')
    ...
    if(reasons.length){const objectFocus=reasons.includes('object_interaction'),highRelation=objectFocus||reasons.includes('relationship_change')||reasons.includes('hand_dog_interaction'),salience=objectFocus ? .99 : (reasons.includes('petting_signal') ? .96 : (highRelation ? .9 : .65)),semantic_hint=objectFocus?'Inspect specifically for a hand presenting an object and for dog mouth contact, pickup, holding, tugging, playing, or dropping. Report none if not clearly visible.':undefined;candidates.push({id:`candidate_${index}`,start:Math.max(0,previous.time-.6),end:Math.min(duration||current.time,current.time+.9),reasons,subjects,salience,semantic_hint})}
  }
...
function parseVisionJson(value){
  ...
  for(const start of starts){const opener=normalized[start],closer=opener==='{'?'}':']',end=normalized.lastIndexOf(closer);if(end<=start)continue;let candidate=normalized.slice(start,end+1).replace(/,\s*([}\]])/g,'$1');const attempts=[candidate,candidate.replace(/([{,]\s*)'([^'\\]+)'\s*:/g,'$1"$2":').replace(/:\s*'([^'\\]*)'\s*([,}\]])/g,':"$1"$2')];for(const attempt of attempts){try{return JSON.parse(attempt)}catch{}}
  }
  return null
}

const explicitName=value=>typeof value==='string'?value:(typeof value?.name==='string'?value.name:null)
const soleReferenceKey=value=>{if(!value||typeof value!=='object'||Array.isArray(value))return null;const keys=Object.keys(value).filter(key=>/^(?:subject|person)_\d+$/.test(key));return keys.length===1?keys[0]:null}
function normalizeExplicitVisionEvent(raw){if(!raw||typeof raw!=='object'||Array.isArray(raw)||isVisionSchemaEcho(JSON.stringify(raw)))return {event:raw,applied:false};const actorNode=raw.actor_ref&&typeof raw.actor_ref==='object'?raw.actor_ref:null,targetNode=raw.target_ref&&typeof raw.target_ref==='object'?raw.target_ref:null,actor=explicitName(raw.actor_ref)||explicitName(raw.actor)||explicitName(raw.subject)||explicitName(actorNode?.actor)||soleReferenceKey(actorNode),action=typeof raw.action==='string'?raw.action:(typeof raw.action?.action==='string'?raw.action.action:(typeof actorNode?.action==='string'?actorNode.action:actorNode?.action?.action)),target=explicitName(raw.target_ref)||explicitName(raw.target)||explicitName(targetNode?.target)||explicitName(actorNode?.target_ref?.target),confidence=[raw.confidence,targetNode?.confidence,targetNode?.target?.confidence,actorNode?.confidence,actorNode?.target_ref?.confidence,actorNode?.target_ref?.target?.confidence].find(value=>value!==undefined&&value!==null&&value!=='');if(!actor&&!action)return {event:raw,applied:false};const event={...raw};let applied=false;if(actor&&raw.actor_ref!==actor){event.actor_ref=actor;applied=true}if(action&&raw.action!==action){event.action=action;applied=true}if(target&&raw.target_ref!==target){event.target_ref=target;applied=true}if(confidence!==undefined&&raw.confidence!==confidence){event.confidence=confidence;applied=true}return {event,applied}}

export function normalizeVisionStructuredPayload(value){if(typeof value==='string'&&isVisionSchemaEcho(value))return {payload:null,applied:false};const parsed=parseVisionJson(value);if(!parsed)return {payload:null,applied:false};let payload,applied=false;if(Array.isArray(parsed))payload={events:parsed};else if(Array.isArray(parsed.events))payload=parsed;else{const normalized=normalizeExplicitVisionEvent(parsed);if(!normalized.event?.action)return {payload:null,applied:false};payload={events:[normalized.event]};applied=normalized.applied}const events=payload.events.map(raw=>{const normalized=normalizeExplicitVisionEvent(raw);applied=applied||normalized.applied;return normalized.event});return {payload:{...payload,events},applied}}
export function parseVisionStructuredPayload(value){return normalizeVisionStructuredPayload(value).payload}
...
export function validateStructuredVision(value,{interval={start:0,end:0},knownSubjects=[]}={}){
  ...
  const {payload:parsed,applied:normalizationApplied}=normalizeVisionStructuredPayload(value)
  ...
  return {valid:errors.length===0||events.length>0,events,context:parsed.context&&typeof parsed.context==='object'?parsed.context:null,errors,normalization_applied:normalizationApplied,normalized_payload:normalizationApplied?parsed:null}
}
...
const OBJECT_SEMANTIC_ACTIONS=new Set(['object_presented','mouth_contact','picking_up','holding','carrying','playing','tugging','dropping'])
export function deduplicateSupportTransitions(events=[],gapSeconds=3){const ordered=events.map(event=>makeEvent(event)).sort((a,b)=>a.start-b.start),result=[],rejected=[],lastByActor=new Map();for(const event of ordered){if(!['jumping_on','jumping_off'].includes(event.action)){result.push(event);continue}const previous=lastByActor.get(event.actor);if(previous&&previous.action===event.action&&event.start-previous.end<=gapSeconds){previous.end=Math.max(previous.end,event.end);previous.confidence=Math.max(previous.confidence,event.confidence);previous.importance=Math.max(previous.importance,event.importance);previous.rawIds=[...new Set([...(previous.rawIds||[]),event.id,...(event.rawIds||[])])];rejected.push({event,reason:'duplicate_support_episode'});continue}result.push(event);lastByActor.set(event.actor,event)}return {events:result,rejected}}
export function reconcileV3Events(events=[],{sessionDuration=0,maxEvents=12}={}){
  const rejected=[],safe=[],normalized=events.map(event=>makeEvent(event)),objectSemantics=normalized.filter(event=>String(event.source||'').startsWith('visionpsy-v3')&&OBJECT_SEMANTIC_ACTIONS.has(event.action))
  for(const event of normalized){const fromId=event.from?.surface_id,toId=event.to?.surface_id,fromFurniture=Boolean(fromId)||event.from?.surface&&!['floor','ground'].includes(event.from.surface),toFurniture=Boolean(toId)||event.to?.surface&&!['floor','ground'].includes(event.to.surface);if(NOISE_ACTIONS.has(event.action)||event.confidence<.48)continue;if(event.action==='petting'&&event.source==='relation-v3'&&objectSemantics.some(candidate=>candidate.actor===event.actor&&candidate.start<=event.end+1&&event.start<=candidate.end+1)){rejected.push({event,reason:'overlapping_object_semantic_priority'});continue}if(event.action==='scene_change'&&event.source==='surface-v3'){rejected.push({event,reason:'surface_scene_change'});continue}if(['jumping_on','jumping_off'].includes(event.action)&&((fromId&&toId&&fromId===toId)||(fromFurniture&&toFurniture))){rejected.push({event,reason:'invalid_support_transition'});continue}safe.push(event)}
  const supportDeduplicated=deduplicateSupportTransitions(safe);rejected.push(...supportDeduplicated.rejected);const merged=mergeEvents(mergePettingEpisodes(supportDeduplicated.events),{gapSeconds:1.4,maxContinuousGapSeconds:3.2})
...
export function selectDeepV3StoryEvents(events=[],{sessionDuration=0,maxEvents=12}={}){const valid=mergePettingEpisodes(events).filter(event=>event.action!=='scene_change'&&event.confidence>=.48),hasSemantic=valid.some(event=>HIGH_NARRATIVE_ACTIONS.has(event.action)&&!SUPPORT_ACTIONS.has(event.action)),priority=event=>{const posture=event.action.endsWith('_up')||event.action.endsWith('_down'),base=HIGH_NARRATIVE_ACTIONS.has(event.action)?1:(posture ? .2 : .45);return base+event.importance*.45+event.confidence*.3},ranked=valid.sort((a,b)=>priority(b)-priority(a)),semantic=ranked.filter(event=>HIGH_NARRATIVE_ACTIONS.has(event.action)&&event.action!=='petting'&&!SUPPORT_ACTIONS.has(event.action)),support=ranked.filter(event=>SUPPORT_ACTIONS.has(event.action)),petting=ranked.filter(event=>event.action==='petting'),low=ranked.filter(event=>!HIGH_NARRATIVE_ACTIONS.has(event.action)),chosen=[];for(const pool of [semantic,support.slice(0,hasSemantic?2:maxEvents),petting.slice(0,2),low])for(const event of pool)if(chosen.length<maxEvents&&!chosen.some(item=>item.id===event.id))chosen.push(event);return chosen.sort((a,b)=>a.start-b.start)}
