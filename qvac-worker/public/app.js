...
async function runDeepRecordedAnalysis(){
  ...
  for(let frameIndex=0;frameIndex<totalFrames;frameIndex++){
    ...
    for(const dog of stableDogs){const contactingHands=hands.filter(points=>points.some(point=>point.x>=dog.box[0]-.04&&point.x<=dog.box[2]+.04&&point.y>=dog.box[1]-.04&&point.y<=dog.box[3]+.04)),contact=contactingHands.length>0,signal=handRelations.update({subjectId:dog.subject_id,time,contact,handPoints:contactingHands});if(contact)relations.push({type:'hand_dog',first:dog.subject_id,second:'person_1',distance:0,changed:true,persistent:signal.active});if(signal.event){frameEvents.push(signal.event);semanticEvents.push(signal.event)}}
